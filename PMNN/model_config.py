...
class PINNConfig(BaseConfig):
    ...
    def optimize_one_epoch(self):
        ...
        self.u = Lu
        # 归一化
        self.u = self.data_loader(self.detach(self.u/torch.norm(self.u, p=2)), requires_grad=False)

        # u = u/torch.norm(u)
        loss_PM = self.loss_func(u, self.u)
        ...
        if np.remainder(self.nIter, loss_remainder) == 0:
            ...
            log_str = str(self.optimizer_name) + ' Iter ' + str(self.nIter) + ' Loss ' + str(loss) +\
                ' lambda_ ' + str(lambda_) +' loss_PM ' + str(loss_PM) + \
